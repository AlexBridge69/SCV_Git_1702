...
## 5. Добаление файла к отслеживанию
...
Если изменена одна и та же часть файла, то может возникнуть конфликт, который потребует участия пользователя. Для отмены слияния и конфликта используем `git merge --abort`, но до commit.
...
* Сравнить версии
## 11. Удаление веток
`git branch -d <имя удаляемой ветки>` - если не было слияния, то выдаст ошибку и предложит перенести информацию в основную ветку.
...
* Compare changes
## 14. Удаление веток
Данная операция выполняется командой `git branch -d <имя удаляемой ветки>` - если не было слияния, то выдаст ошибку и предложит перенести информацию в основную ветку.

Для принудительного удаления ветки, которая не была слита с главной надо ввести команду: `git branch -D <имя удаляемой ветки>`.
## 15. Сокращённые команды Git
Для упрощения работы можно сделать сокращённые версии команд git. Для создания короткой версии любой команды Git, необходимо ввести в Терминале следующую команду:
```
git config --global alias.<сокращение> '<Название сокрщяемой команды>'
```
Полный список сокращений можно посмотреть с помощью команды: `git config --global --list`.
## 16. Ошибки при работе с коммитами
Если перешли в один из предыдущих коммитов, сделали там изменения, закомитили их и перешли в последнюю версию, то в ней изменения не отобразятся.
Чтобы отобразить в ветке _master_ изменения из другого коммита, необходимо:
1. Вводим команду `git branch <имя ветки> <номер непоказываемого коммитва>`.
2. Переходим на ветку _master_.
3. Сливаем её с последней созданной веткой.
4. При ниобходимости решаем конфликт.
5. Делаем коммит изменения в главной ветке.
# Работа с удалёнными репозиториями
Работа с сервисом GitHub в общем случае делится на 2 группы:
1. Выгрузка своих проектов (репозиториев) непосредственно на платформу;
2. Обмен репозиториями (загрузка их себе) и их редактирование.
Для того, чтобы выложить свой проект на GitHub, необходимо:
* Зарегистрироваться на сайте GitHub;
* Создать свой локальный проект;
* В личном кабинете на портале щёлкнуть по конопке _New repisitory_ и следовать инструкции;
* Связать свой локальный репозитроий и удаленный с помощью команды `git remote add <имя репозитория> <его адрес, смотреть по кнопке "Code">` 
* Отправить изменения в удаленный репозиторий по команде `git push <имя репозитория>`. 
```
При первом использовании команды push git может потребовать авторизоваться на портале для подтверждения права внесения изменений.
```
**Для работы и удалеными репозиториями и их выгрузки существует 2 варианта действий:**
1. Клонировать удалённый репозиторий в свой локальный по команде `git clone <адрес репозитория>`, внести необходимые изменения и выполнить команду `git push <имя репозитория>`.
```
Если изменения били созданы заранее, то при вставке данных из локального репозитория в клонированный будет утеряна история коммитов.
```
2. Второй вариант позволяет исключить потерю истории, т.к. здесь происходит связывание локального и удалённого репозиториев и, соответственно, их историй. Производится с помощью команды `git remote add <имя> <адрес>`. Данная команда позволяет не загружать весь удаленный проект целиком, а только конкретные ветки с помощью команды  `git pull <имя> <ветка>`
Также эта команда позволяет стягивать все изменения, которые были проведены в удаленном репозитории.

**Чтобы начатть работу с удаленным репозиторием, необходимо:**
1. Зарегистрироваться на сайте GitHub;
2. Скопировать адрес публичного проекта, если проект приватный, то добавить его к себе в профиль кнопкой **Fork** и затем скопировать адрес;
3. Добавть его в свой локальный репозиторий командой  `git clone <адрес репозитория>` или `git remote add <имя> <адрес>`

__Далее в зависимости от целей:__

4. При необходимости стянуть изменения к себе командой  `git pull <имя> <ветка>`
5. Выполнить необходимую работу;
6. Выгрузить измененный репозиторий обратно на платформу командой `git push <имя репозитория>`;
7. Если исходный проект был закрытый, то по подсказкам платформы выполнить `pull request`, то есть предложить авторам проекта свои изменения.
